(() => {
  ...
  function renderResults() {
    ...
    $("results").innerHTML=state.matches.slice(0,state.visible).map(c=>{
      ...
      return `<article class="result-card"><div class="result-top"><h3><span>${cleanText(c.code)}</span>${cleanText(c.title)}</h3><span class="level">${c.level==="G"?"Graduate":"Undergrad"}</span></div><div class="course-metrics">${rating}${hours}</div>${preparation}<div class="meetings">${meetings}</div>${bus}<p class="description">${cleanText(c.description||"See the MIT subject listing for details.")}</p><button class="add-mit ${already?"added":""}" data-mit="${cleanText(c.code)}" ${already?"disabled":""}>${already?"Added to schedule ✓":"+ Add to schedule"}</button><a class="result-link" href="${cleanText(c.url)}" target="_blank" rel="noopener">Official listing ↗</a></article>`;
